...
let SCRIPT_URL = localStorage.getItem('tenant_script_url') || "https://script.google.com/macros/s/AKfycbxBkcaqATf2TwWYPp9g1BotA3YfJqmPPdE1euJZmWXMSD9xdYZfeZols7T5H7jqVd7lBw/exec";
...
let orders = [
    { 
        ...
        phone: '628123456789', 
        service: 'Cuci Kering + Setrika (2x)', 
        total: 16000, 
        cashier: 'Sistem', 
        status: 'Diproses', 
        method: 'Tunai / Cash', 
        ...
        cashPaid: 20000,
        cashChange: 4000,
        itemsDetail: [{ name: 'Cuci Kering + Setrika', price: 8000, qty: 2 }],
        date: new Date().toISOString() 
    }
];

let expenses = []; // Menyimpan riwayat catatan pengeluaran

// SESSION CONTROL LOGIC
window.addEventListener('DOMContentLoaded', () => {
    // Terapkan nama tenant ke UI saat aplikasi pertama kali dimuat
    updateTenantUI();
...
function showMainApp() {
    ...
    renderOrders();
    calculateFinance();
    loadSettingsUI(); // Memuat pengaturan Multi-Tenant
    setDefaultDates();
                ...
                orders = cloudData.transactions.map(t => {
                    ...
                    if (t.itemsDetail) {
                        try {
                            ...
                            console.log("Gagal mengurai itemsDetail pada nota:", t.id, e);
                            parsedItems = [];
                        }
                    }
                    ...
                    return {
                        ...
                        // ✨ INI DIA PERBAIKANNYA: Menangkap data estimasi dari cloud!
        ...
        htmlContent += `
            <div onclick="selectServiceToCart('${item.id}')" class="bg-white p-5 rounded-2xl border cursor-pointer transition-all flex flex-col justify-between h-36 relative active:scale-95 ${borderStyle}">
                ...
                <div class="flex justify-between items-start">
                    ...
                    ${!isCustomMenu ? `<i class="fa-solid ${item.icon} ${isSelected ? 'text-cyan-400' : 'text-slate-300'} text-base"></i>` : '<div class="w-2"></div>'}
                </div>
                <div>
            ...
            <div class="bg-white border-2 border-teal-400 p-4 rounded-2xl shadow-xs w-full flex flex-col gap-3 relative">
                ...
                <div class="flex justify-between items-center w-full">
                    ...
                    <button 
                        ...
                        class="text-rose-500 bg-rose-50 w-6 h-6 rounded-full hover:bg-rose-100 transition-all flex items-center justify-center shrink-0 shadow-2xs" 
                        title="Hapus Layanan"
                    >
...
<input 
    ...
    min="0.01" 
    step="${isKiloan ? '0.1' : '1'}" 
    oninput="updateCartQty(${index}, this.value)" 
    class="w-14 bg-slate-50 border border-slate-200 rounded-xl p-1 font-extrabold text-slate-700 text-center outline-none focus:border-[#40E0D0] transition-all text-xs">
...
function deleteCartItem(index) {
    ...
    // Hapus 1 item dari array keranjang berdasarkan indeksnya
    cart.splice(index, 1);
    
    // Segarkan ulang tampilan keranjang dan grid menu produk agar status seleksinya sinkron
    renderCart();
    renderServicesGrid();
...
function updateCartQty(index, val) {
    ...
    // ✨ FIX: Update angka nominal di dalam kotak layanannya!
    ...
    // Kalkulasi matematika total tagihan bawah
    let totalAkhir = 0;
    cart.forEach((item) => {
...
function toggleNewCustomerInput() {
    ...
    // Menentukan state berdasarkan status checked pada toggle switch
    ...
    if (isNewCustomerMode) {
        ...
        // Bersihkan inputan jika user mematikan sakelar kembali
...
function processCheckout() {
    ...
    // Validasi: Hapus item yang qty-nya kosong atau 0 sebelum dihitung
    ...
    // ✨ INI DIA KODE BARUNYA (MENGAMBIL TANGGAL DARI INPUT KASIR)
    ...
    const checkoutPayload = {
        ...
        date: inputOrderDate,               // <-- Memasukkan tanggal masuk ke payload
        estimatedPickup: inputPickupDate    // <-- Memasukkan estimasi ambil ke payload
    };
    ...
    // RESET FORM KERANJANG KE KONDISI AWAL
    cart = [];
    renderCart();
    ...
    // ✨ RESET KEMBALI TANGGAL KE WAKTU SAAT INI UNTUK PELANGGAN SELANJUTNYA
    setDefaultDates();
...
function openReceiptModal(order) {
    // 1. Mengisi data teks tanggal di baris atas (Di bawah Hub)
    if(document.getElementById('nota-date')) {
        document.getElementById('nota-date').innerText = formatTanggalIndo(order.date);
    }
    
    // 2. Mengisi teks ke alamat yang tepat di dalam tabel rincian (Solusi Utama)
    if(document.getElementById('nota-date-in')) {
        document.getElementById('nota-date-in').innerText = formatTanggalIndo(order.date);
    }
    if(document.getElementById('nota-estimasi')) {
        document.getElementById('nota-estimasi').innerText = order.estimatedPickup ? formatTanggalIndo(order.estimatedPickup) : "-";
    }

    // Sisanya tetap sama seperti kode bawaanmu:
    ...
    if(document.getElementById('nota-service')) {
        document.getElementById('nota-service').innerText = order.service;
    }
    if(document.getElementById('nota-price')) {
...
function openLiveTrackingPreview(orderId) {
    ...
    // ✨ INI BAGIAN D: Nempel data tanggal ke HTML layar Pelacakan (Track)
    if(document.getElementById('track-date-in')) {
        document.getElementById('track-date-in').innerText = formatTanggalIndo(order.date);
    }
    if(document.getElementById('track-date-out')) {
        document.getElementById('track-date-out').innerText = order.estimatedPickup ? formatTanggalIndo(order.estimatedPickup) : "-";
    }
...
function calculateFinance() {
    ...
    // 1. Hitung total keseluruhan (Semua Metode)
    const totalIncome = filteredOrders.reduce((sum, o) => sum + o.total, 0);
    const totalNotes = filteredOrders.length;

    // 2. Hitung spesifik per metode pembayaran (Mencocokkan string dari pilihan kasir)
    ...
    // Kirim data nominal ke elemen HTML
    ...
    // Render daftar log riwayat transaksi
    if (logList) {
...
async function printBluetoothReceipt() {
    try {
        ...
        const notaDate = new Date(currentOrderData.date).toLocaleString('id-ID');
        ...
        // Header nota menyesuaikan nama tenant yang disimpan
        printPayload.push(CMD_CENTER, CMD_BOLD_ON, text(`====== ${tenantName.toUpperCase()} ======"`), CMD_BOLD_OFF);
        printPayload.push(text("Hub.085659679645"));
...
function saveTenantSettings() {
    const newUrl = document.getElementById('setting-script-url').value.trim();
    const newName = document.getElementById('setting-tenant-name').value.trim();
    const newPhone = document.getElementById('setting-tenant-phone').value.trim();

    if(!newUrl) return alert("❌ URL Database Google Sheets tidak boleh kosong!");
    if(!newName) return alert("❌ Nama Toko Laundry tidak boleh kosong!");
    ...
    localStorage.setItem('tenant_phone', tenantPhone);

    updateTenantUI();
    
    triggerNotification(`✅ Pengaturan Toko "${tenantName}" berhasil disimpan!`);
    
    // Muat ulang data dari database yang baru
    loadDataFromCloud(); 
}
...
// ===============================================================
// FITUR UNDUH KODE GOOGLE APPS SCRIPT LOKAL
// ===============================================================
function downloadGSCode() {
    // Menambahkan teks pesanan dan link WhatsApp
    // \n digunakan untuk membuat baris baru (enter) di dalam file teks
    const gsCode = "Kalau mau kode GS, silakan order ke admin melalui link berikut:\nhttps://wa.me/6285659679645";

    // Mengubah string menjadi format file yang bisa diunduh browser
    ...
    // Memicu klik otomatis pada elemen <a> untuk memulai unduhan
    document.body.appendChild(downloadLink);
    downloadLink.click();
    
    // Membersihkan elemen sesudah selesai
...
function renderExpenses(dataToRender = expenses) {
    ...
    if (dataToRender.length === 0) {
        logList.innerHTML = `<p class="text-center italic text-slate-400 py-4">Belum ada catatan pengeluaran.</p>`;
        return;
    }
...
function saveNewExpense() {
    ...
    // Panggil hitung ulang agar UI langsung beradaptasi dengan filter yang aktif
    calculateExpenses(); 
...
// Fungsi Helper: Set waktu awal & format tanggal
