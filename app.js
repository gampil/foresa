    ...
    fetch(`${SCRIPT_URL}?action=read`)
        ...
        .then(cloudData => {
            ...
            if (cloudData.transactions && cloudData.transactions.length > 0) {
                // Kita pastikan setiap data transaksi dari cloud menyertakan properti date
                orders = cloudData.transactions.map(t => {
                    return {
                        ...
                        method: t.method,
                        status: t.status,
                        // Ambil properti date dari cloud, jika tidak ada pakai waktu saat ini sebagai pengaman
